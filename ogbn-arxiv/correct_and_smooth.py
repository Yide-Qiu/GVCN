...
def main(a,b):
    ...
    argparser.add_argument("--alpha1", type=float, default=a, help="alpha1")
    argparser.add_argument("--n-prop1", type=int, default=10)
    argparser.add_argument("--alpha2", type=float, default=b, help="alpha2")
    ...
    for pred_file in glob.iglob(args.pred_files):
        # pdb.set_trace()
        # if pred_file == "./checkpoint/best_pred_run7.pt":
        #     continue
...
def onjective(trail):
    alpha1 = trail.suggest_float('alpha1',0,1,step=0.001)
    alpha2 = trail.suggest_float('alpha2',0,1,step=0.001)
    test_acc = main(alpha1,alpha2)
    return test_acc
...
if __name__ == "__main__":
    ...
    study.optimize(onjective, n_trials=5000)
